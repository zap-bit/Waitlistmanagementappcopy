...
export function assertConfig(): void {
  ...
  if ((config.supabaseUrl && !config.supabaseServiceRoleKey) || (!config.supabaseUrl && config.supabaseServiceRoleKey)) {
    throw new Error('SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must either both be set or both be empty');
  }
}

export const supabaseConfigured = Boolean(config.supabaseUrl && config.supabaseServiceRoleKey);
