...
export function StaffDashboard({
  ...
  useEffect(() => {
    const pollEvents = () => {
      ...
      if (selectedEvent) {
        const updatedEvent = [...activeEvents, ...archived].find(
      ...
      {currentPage === "home" ? (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="text-xl font-bold text-gray-800 mb-2 pr-44">
                              {event.name}
                            </h3>
              ...
              {attractions.map((attraction) => {
                ...
                const liveWaitlistCount = selectedEvent
                  ? waitlist
                      .filter((e) => {
                        // Filter by event ID
                        if (e.eventId !== selectedEvent.id) return false;
                        
                        // For multi-queue capacity events, also filter by queue ID
                        const isSingleQueue = 
                          selectedEvent.type === "capacity-based" &&
                          (selectedEvent as CapacityBasedEvent).queueMode === "single";
                        
                        if (!isSingleQueue && attraction.id !== "default-single-queue") {
                ...
                if (selectedEvent && selectedEvent.type === "capacity-based") {
                  ...
                  if (capacityEvent.queueMode === "single") {
                    manualOffset = capacityEvent.manualOffset || 0;
                  } else {
                    // Multi-queue mode
                    const queue = capacityEvent.queues?.find(q => q.id === attraction.id);
                    manualOffset = queue?.manualOffset || 0;
                  }
                }
                
                // Actual queue size = live count + manual offset
                const actualQueueSize = liveWaitlistCount + manualOffset;
                          ...
                          <div className="flex items-center justify-between">
                            <button
                              onClick={() => {
                                ...
                                const capacityEvent = selectedEvent as CapacityBasedEvent;
                                const currentOffset = manualOffset;
                                const newOffset = Math.max(-(liveWaitlistCount), currentOffset - 1);
                                ...
                                if (capacityEvent.queueMode === "single") {
                                  ...
                                  const updatedQueues = capacityEvent.queues?.map(q =>
                                    q.id === attraction.id
                                      ? { ...q, manualOffset: newOffset }
                                      : q
                                  );
                                  updateEvent(selectedEvent.id, {
                                ...
                                const updatedEvents = getActiveEvents();
                                const refreshedEvent = updatedEvents.find(e => e.id === selectedEvent.id);
                                if (refreshedEvent) {
                            ...
                            <button
                              onClick={() => {
                                ...
                                const capacityEvent = selectedEvent as CapacityBasedEvent;
                                const currentOffset = manualOffset;
                                const maxPossibleOffset = attraction.queueCapacity - liveWaitlistCount;
                                const newOffset = Math.min(maxPossibleOffset, currentOffset + 1);
        ...
        <div className="flex-1 overflow-auto flex items-center justify-center p-6">
          <SimpleCapacityTracker
            ...
            onIncrement={() => {
              ...
              if (
                simpleEvent.currentCount < simpleEvent.capacity
              ) {
        ...
        <div className="flex-1 overflow-auto">
          <div className="p-4 space-y-4">
            ...
            {waitlist.filter(
              (e) =>
                e.type === listView && e.eventId === selectedEvent?.id,
            ).length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                <p>
                  {listView === "reservation"
      ...
      {showEditEventModal && eventToEdit && (
        <CreateEventModal
          ...
          onCreateEvent={(event) => {
            ...
            // Refresh events list
            const updatedActiveEvents = getActiveEvents().filter(
              (e) => e.businessId === user.businessId,
            );
            ...
            if (selectedEvent && selectedEvent.id === event.id) {
              ...
              if (event.type === "capacity-based") {
                ...
                if (capacityEvent.queueMode === "multiple" && capacityEvent.queues) {
                  setAttractions(
                    capacityEvent.queues.map((queue) => ({
                      id: queue.id,
                      name: queue.name,
                ...
                if (newTableCount !== totalTables) {
                  ...
                  if (existingTables && existingTables.length > 0) {
                    ...
                    if (newTableCount > existingTables.length) {
                      ...
                      for (let i = existingTables.length; i < newTableCount; i++) {
                        const row = Math.floor(i / cols);
                        const col = i % cols;
                        updatedTables.push({
                          id: i + 1,
                      ...
                      updatedTables = existingTables.slice(0, newTableCount);
                    }
                  } else {
                    // Create new tables
                    for (let i = 0; i < newTableCount; i++) {
