...
export function Profile({ user, onClose, onLogout }: ProfileProps) {
  const [profileData, setProfileData] = useState<ProfileData>(() => {
    if (typeof window !== 'undefined') {
      const saved = appStorage.getItem(`profile_${user.id}`);
      if (saved) {
        try {
  ...
  const handleSave = () => {
    if (typeof window !== 'undefined') {
      appStorage.setItem(`profile_${user.id}`, JSON.stringify(profileData));
...
export function getSavedProfile(userId: string): ProfileData | null {
  if (typeof window !== 'undefined') {
    const saved = appStorage.getItem(`profile_${userId}`);
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
