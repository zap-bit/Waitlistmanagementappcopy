...
export function AttendeeView({ onLogout, waitlist, addToWaitlist, removeFromWaitlist, updateWaitlistEntry, allWaitlistEntries, tables, user }: AttendeeViewProps) {
  ...
  const [myWaitlistIds, setMyWaitlistIds] = useState<string[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = appStorage.getItem('myWaitlistIds');
      if (saved) {
        try {
  ...
  const [isOnline, setIsOnline] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = appStorage.getItem('attendeeIsOnline');
      if (saved) {
        try {
          return JSON.parse(saved);
        } catch (e) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (myWaitlistIds.length > 0) {
        appStorage.setItem('myWaitlistIds', JSON.stringify(myWaitlistIds));
      } else {
        appStorage.removeItem('myWaitlistIds');
      }
    }
  }, [myWaitlistIds]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      appStorage.setItem('attendeeIsOnline', JSON.stringify(isOnline));
    }
  }, [isOnline]);
