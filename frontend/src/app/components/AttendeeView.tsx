...
export function AttendeeView({
  ...
  // Load available events on mount and poll for updates (exclude simple-capacity/attendance-tracking events)
  useEffect(() => {
    const loadEvents = () => {
      const events = getStoredEvents().filter(
        (e) =>
          e.status === "active" && e.type !== "simple-capacity",
      );
  ...
  useEffect(() => {
    // Don't run if events haven't loaded yet
  ...
  const estimatedWaitMinutes = myEntry
    ? calculateDynamicWaitTime(myEntry, allWaitlistEntries)
    : 0;
  ...
  useEffect(() => {
    if (selectedWaitlistId && !myEntry) {
      ...
      toast.info(
        "You have been seated or removed from the waitlist",
      );
    }
    ...
    try {
      ...
      if (parsed.type === "waitlist-event" && parsed.eventId) {
        const event = availableEvents.find(
          (e) =>
            e.id === parsed.eventId ||
            e.eventCode === parsed.eventCode,
        );

        if (event) {
          setSelectedEvent(event);
          setEventCode(parsed.eventCode || event.eventCode || "");
  ...
  const handleJoinManually = () => {
    ...
    if (reservationTime) {
      ...
      const dateToUse = reservationDate
